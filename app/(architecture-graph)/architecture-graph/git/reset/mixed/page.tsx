...
// ============================================================================
// Git Mixed Reset Architecture
// ============================================================================

export default function GitMixedResetPage() {
  return (
    <div className="w-full max-w-6xl mx-auto p-6 lg:p-8 flex flex-col gap-6">
      <GitMixedResetDiagram />
    </div>
  );
}

function GitMixedResetDiagram() {
  ...
  const renderFileState = (
    ...
    if (currentStep === 0) {
      ...
      // After reset --mixed: HEAD moved to C2, C3 changes in working dir (unstaged)
      addRepoCommit(svg, "C1: Initial", 0);
      addRepoCommit(svg, "C2: Feature", 1);

      // Files from C3 now in working directory (unstaged)
      addWorkingFile(svg, "app.js (modified)", 0);
      addWorkingFile(svg, "styles.css (new)", 1);
  ...
  const handleResetState = () => {
    ...
    if (svgRef.current) {
      const svg = d3.select(svgRef.current);
      svg.select(".commit-C3").attr("fill", "#3b82f6").attr("opacity", 1);
      svg
        .select(".c2-c3-link")
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              ...
              Git Mixed Reset (Default)
            </h1>
            <p className="text-slate-400 text-sm mt-1">
              Moves HEAD and resets Index, but preserves changes in Working
              Directory
            </p>
          </div>
          ...
          {step === 1 && (
            <div>
              <strong className="text-green-400">
                After git reset HEAD~1 (--mixed is default):
              </strong>
              <ul className="list-disc list-inside mt-2 space-y-1">
                <li>HEAD moved back to C2 (C3 becomes unreachable)</li>
                <li>
                  <strong>Staging Area:</strong> Reset to match C2 (empty)
                </li>
                <li className="text-yellow-400">
                  <strong>Working Directory:</strong> Contains changes from C3
                  (unstaged)
                </li>
                <li className="text-blue-400">
                  💡 Changes appear as "modified but not staged"
                </li>
              </ul>
            </div>
          )}
