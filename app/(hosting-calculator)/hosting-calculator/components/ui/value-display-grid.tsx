...
export function ValueDisplayGrid({
  title,
  items,
  columns = 2,
  className,
}: ValueDisplayGridProps) {
  return (
    <div
      className={cn(
        "rounded border border-border/60 bg-card/80 p-4 md:p-5",
        className
      )}
    >
      {title && (
      ...
      >
        {items.map((item, idx) => (
          <div
            key={idx}
            className="flex flex-col rounded bg-muted/30 px-4 py-3"
          >
