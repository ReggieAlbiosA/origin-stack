...
const highlightStyles: Record<
  NonNullable<ProviderCostCardProps["highlight"]>,
  string
> = {
  default: "",
        ...
        {breakdown && breakdown.length > 0 && (
          <ul className="mt-3 space-y-2 text-sm">
            {breakdown.map((item, idx) => (
              <li
                key={idx}
                className="flex items-center justify-between rounded bg-muted/40 px-3 py-2 text-muted-foreground"
              >
