...
export function CalculatorProvider<TInputs, TPlan extends Plan>({
  ...
  const [inputs, setInputsState] = React.useState<TInputs>(
    config.defaultInputs
  );
  const [plan, setPlanState] = React.useState<TPlan>(
    Object.keys(config.planLimits)[0] as TPlan
  );
  ...
  // Set inputs wrapper
  const setInputs = React.useCallback(
    (newInputs: TInputs | ((prev: TInputs) => TInputs)) => {
      setInputsState((prev) => {
  ...
  const handleTeamMembersChange = React.useCallback(
    (members: number) => {
  ...
  } as CalculatorContextValue<TInputs, TPlan> & {
    teamMembers: number;
...
export function useCalculator<TInputs = any, TPlan extends Plan = Plan>() {
  ...
  return context as CalculatorContextValue<TInputs, TPlan> & {
    teamMembers?: number;
    setTeamMembers?: (members: number) => void;
  };
}
