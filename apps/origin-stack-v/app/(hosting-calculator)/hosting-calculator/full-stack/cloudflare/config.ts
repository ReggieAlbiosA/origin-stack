...
function calculateCloudflareCost(
  ...
  const WORKER_REQUESTS_PRICE = 0.3 / 1_000_000;
  ...
  const D1_ROWS_WRITTEN_PRICE = 1.0 / 1_000_000;
  const D1_STORAGE_INCLUDED = 5; // GB
  const D1_STORAGE_PRICE = 0.75; // per GB/month

  // Images
  const IMAGES_TRANSFORMS_INCLUDED = 5_000;
  const IMAGES_TRANSFORMS_PRICE = 0.5 / 1_000;
  const IMAGES_STORED_PRICE = 5 / 100_000; // per 100k images/month
  const IMAGES_DELIVERED_PRICE = 1 / 100_000; // per 100k delivered/month

  // Workers
  const workerRequestsCost =
    Math.max(0, inputs.workerRequests - WORKER_REQUESTS_INCLUDED) *
    WORKER_REQUESTS_PRICE;
  const cpuTimeCost =
    Math.max(0, inputs.cpuTimeMs - CPU_TIME_INCLUDED) * CPU_TIME_PRICE;

  // KV
  const kvReadsCost =
    Math.max(0, inputs.kvReads - KV_READS_INCLUDED) * KV_READS_PRICE;
  const kvWritesCost =
    Math.max(0, inputs.kvWrites - KV_WRITES_INCLUDED) * KV_WRITES_PRICE;
  const kvStorageCost =
    Math.max(0, inputs.kvStorageGb - KV_STORAGE_INCLUDED) * KV_STORAGE_PRICE;

  // R2
  const r2StorageCost =
    Math.max(0, inputs.r2StorageGb - R2_STORAGE_INCLUDED) * R2_STORAGE_PRICE;
  const r2ClassACost =
    Math.max(0, inputs.r2ClassAOps - R2_CLASS_A_INCLUDED) * R2_CLASS_A_PRICE;
  const r2ClassBCost =
    Math.max(0, inputs.r2ClassBOps - R2_CLASS_B_INCLUDED) * R2_CLASS_B_PRICE;

  // D1
  const d1RowsReadCost =
    Math.max(0, inputs.d1RowsRead - D1_ROWS_READ_INCLUDED) * D1_ROWS_READ_PRICE;
  const d1RowsWrittenCost =
    Math.max(0, inputs.d1RowsWritten - D1_ROWS_WRITTEN_INCLUDED) *
    D1_ROWS_WRITTEN_PRICE;
  const d1StorageCost =
    Math.max(0, inputs.d1StorageGb - D1_STORAGE_INCLUDED) * D1_STORAGE_PRICE;

  // Images
  const imageTransformsCost =
    Math.max(0, inputs.imageTransformations - IMAGES_TRANSFORMS_INCLUDED) *
    IMAGES_TRANSFORMS_PRICE;
  ...
  return {
    ...
    breakdown: [
      { label: "Base Subscription", value: BASE_PRICE, category: "Base" },
      {
        ...
        value: workerRequestsCost,
        category: "Workers",
        included: WORKER_REQUESTS_INCLUDED,
      },
      {
        label: "CPU Time",
        value: cpuTimeCost,
        category: "Workers",
        included: `${CPU_TIME_INCLUDED / 1_000_000}M ms`,
      },
      {
        label: "KV Reads",
        value: kvReadsCost,
        category: "KV Storage",
        included: KV_READS_INCLUDED,
      },
      {
        label: "KV Writes",
        value: kvWritesCost,
        category: "KV Storage",
        included: KV_WRITES_INCLUDED,
      },
      {
        label: "KV Storage",
        value: kvStorageCost,
        category: "KV Storage",
        included: `${KV_STORAGE_INCLUDED} GB`,
      },
      {
        label: "R2 Storage",
        value: r2StorageCost,
        category: "R2",
        included: `${R2_STORAGE_INCLUDED} GB`,
      },
      {
        label: "R2 Class A Ops",
        value: r2ClassACost,
        category: "R2",
        included: R2_CLASS_A_INCLUDED,
      },
      {
        label: "R2 Class B Ops",
        value: r2ClassBCost,
        category: "R2",
        included: R2_CLASS_B_INCLUDED,
      },
      {
        label: "D1 Rows Read",
        value: d1RowsReadCost,
        category: "D1",
        included: `${D1_ROWS_READ_INCLUDED / 1_000_000_000}B`,
      },
      {
        label: "D1 Rows Written",
        value: d1RowsWrittenCost,
        category: "D1",
        included: `${D1_ROWS_WRITTEN_INCLUDED / 1_000_000}M`,
      },
      {
        label: "D1 Storage",
        value: d1StorageCost,
        category: "D1",
        included: `${D1_STORAGE_INCLUDED} GB`,
      },
      {
        label: "Image Transforms",
        value: imageTransformsCost,
        category: "Images",
        included: IMAGES_TRANSFORMS_INCLUDED,
      },
      { label: "Images Stored", value: imagesStoredCost, category: "Images" },
      {
        label: "Images Delivered",
        value: imagesDeliveredCost,
        category: "Images",
      },
    ],
  };
