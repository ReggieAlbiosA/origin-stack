...
function calculateVercelCost(
  ...
  const EDGE_REQUESTS_PRICE = 2.0 / 1_000_000; // $2 per 1M requests (after included)
  ...
  const FUNCTION_INVOCATIONS_PRICE = 0.6 / 1_000_000; // $0.60 per 1M (starting at)
  ...
  const SANDBOX_CREATIONS_PRICE = 0.6 / 1_000_000; // $0.60 per 1M (starting at)
  ...
  const WORKFLOW_STEPS_PRICE = 2.5 / 100_000; // $2.50 per 100K
  const WORKFLOW_STORAGE_INCLUDED = 1; // 1 GB included in Hobby
  const WORKFLOW_STORAGE_PRICE = 0.5; // per GB
  ...
  const IMAGE_CACHE_READS_PRICE = 0.4 / 1_000_000; // $0.40 per 1M reads (starting at)
  const IMAGE_CACHE_WRITES_INCLUDED = 100_000; // 100K included in Hobby
  const IMAGE_CACHE_WRITES_PRICE = 4.0 / 1_000_000; // $4 per 1M writes (starting at)
  ...
  const BLOB_SIMPLE_OPS_PRICE = 0.4 / 1_000_000;
  const BLOB_ADVANCED_OPS_INCLUDED = 2_000; // 2K included in Hobby
  const BLOB_ADVANCED_OPS_PRICE = 5.0 / 1_000_000;
  ...
  const EDGE_CONFIG_READS_PRICE = 3.0 / 1_000_000; // $3 per 1M reads
  const EDGE_CONFIG_WRITES_INCLUDED = 100; // 100 included in Hobby
  const EDGE_CONFIG_WRITES_PRICE = 5.0 / 500; // $5 per 500 writes

  // ISR (Incremental Static Regeneration)
  const ISR_READS_INCLUDED = 1_000_000; // 1M included in Hobby
  const ISR_READS_PRICE = 0.4 / 1_000_000; // $0.40 per 1M reads
  const ISR_WRITES_INCLUDED = 200_000; // 200K included in Hobby
  const ISR_WRITES_PRICE = 4.0 / 1_000_000; // $4 per 1M writes

  // Firewall
  const RATE_LIMITING_INCLUDED = 1_000_000; // 1M included in Hobby
  const RATE_LIMITING_PRICE = 0.5 / 1_000_000; // $0.50 per 1M allowed requests (starting at)
  const BOTID_DEEP_ANALYSIS_PRICE = 1.0 / 1_000; // $1 per 1K Deep Analysis checks

  // Edge Requests - $2 per 1M after 10M included (Pro)
  const edgeRequestsCost =
    Math.max(0, inputs.edgeRequests - EDGE_REQUESTS_INCLUDED) *
    EDGE_REQUESTS_PRICE;

  // Fast Data Transfer - $0.15 per GB after 1TB included (Pro)
  const fastDataTransferCost =
    Math.max(0, inputs.fastDataTransfer - FAST_DATA_TRANSFER_INCLUDED) *
    FAST_DATA_TRANSFER_PRICE;

  // Fast Origin Transfer - $0.06 per GB after 10GB included (Hobby)
  const fastOriginTransferCost =
    Math.max(0, inputs.fastOriginTransfer - FAST_ORIGIN_TRANSFER_INCLUDED) *
    FAST_ORIGIN_TRANSFER_PRICE;

  // Vercel Functions
  const functionActiveCpuCost =
    Math.max(0, inputs.functionActiveCpuTime - FUNCTION_ACTIVE_CPU_INCLUDED) *
    FUNCTION_ACTIVE_CPU_PRICE;
  const functionProvisionedMemoryCost =
    Math.max(
      0,
      inputs.provisionedMemory - FUNCTION_PROVISIONED_MEMORY_INCLUDED
    ) * FUNCTION_PROVISIONED_MEMORY_PRICE;
  const functionInvocationsCost =
    Math.max(0, inputs.invocations - FUNCTION_INVOCATIONS_INCLUDED) *
    FUNCTION_INVOCATIONS_PRICE;

  // Vercel Sandbox
  const sandboxActiveCpuCost =
    Math.max(0, inputs.sandboxActiveCpuTime - SANDBOX_ACTIVE_CPU_INCLUDED) *
    SANDBOX_ACTIVE_CPU_PRICE;
  const sandboxProvisionedMemoryCost =
    Math.max(
      0,
      inputs.sandboxProvisionedMemory - SANDBOX_PROVISIONED_MEMORY_INCLUDED
    ) * SANDBOX_PROVISIONED_MEMORY_PRICE;
  const sandboxCreationsCost =
    Math.max(0, inputs.sandboxCreations - SANDBOX_CREATIONS_INCLUDED) *
    SANDBOX_CREATIONS_PRICE;
  const sandboxNetworkCost =
    Math.max(0, inputs.sandboxNetwork - SANDBOX_NETWORK_INCLUDED) *
    SANDBOX_NETWORK_PRICE;

  // Vercel Workflow
  const workflowStepsCost =
    Math.max(0, inputs.steps - WORKFLOW_STEPS_INCLUDED) * WORKFLOW_STEPS_PRICE;
  const workflowStorageCost =
    Math.max(0, inputs.storage - WORKFLOW_STORAGE_INCLUDED) *
    WORKFLOW_STORAGE_PRICE;
  ...
  const imageTransformationsCost =
    Math.max(0, inputs.imageTransformations - IMAGE_TRANSFORMATIONS_INCLUDED) *
    IMAGE_TRANSFORMATIONS_PRICE;

  // Image Cache Operations
  const imageCacheReadsCost =
    Math.max(0, inputs.imageCacheReads - IMAGE_CACHE_READS_INCLUDED) *
    IMAGE_CACHE_READS_PRICE;
  const imageCacheWritesCost =
    Math.max(0, inputs.imageCacheWrites - IMAGE_CACHE_WRITES_INCLUDED) *
    IMAGE_CACHE_WRITES_PRICE;

  // Web Analytics
  const analyticsCost = inputs.webAnalytics * ANALYTICS_EVENTS_PRICE;

  // Blob Storage
  const blobStorageCost =
    Math.max(0, inputs.blobStorageSize - BLOB_STORAGE_INCLUDED) *
    BLOB_STORAGE_PRICE;
  const blobSimpleOpsCost =
    Math.max(0, inputs.blobSimpleOps - BLOB_SIMPLE_OPS_INCLUDED) *
    BLOB_SIMPLE_OPS_PRICE;
  const blobAdvancedOpsCost =
    Math.max(0, inputs.blobAdvancedOps - BLOB_ADVANCED_OPS_INCLUDED) *
    BLOB_ADVANCED_OPS_PRICE;
  const blobDataTransferCost =
    Math.max(0, inputs.blobDataTransfer - BLOB_DATA_TRANSFER_INCLUDED) *
    BLOB_DATA_TRANSFER_PRICE;

  // Edge Config
  const edgeConfigReadsCost =
    Math.max(0, inputs.edgeConfigReads - EDGE_CONFIG_READS_INCLUDED) *
    EDGE_CONFIG_READS_PRICE;
  const edgeConfigWritesCost =
    Math.max(0, inputs.edgeConfigWrites - EDGE_CONFIG_WRITES_INCLUDED) *
    EDGE_CONFIG_WRITES_PRICE;

  // ISR (Incremental Static Regeneration)
  const isrReadsCost =
    Math.max(0, inputs.isrReads - ISR_READS_INCLUDED) * ISR_READS_PRICE;
  const isrWritesCost =
    Math.max(0, inputs.isrWrites - ISR_WRITES_INCLUDED) * ISR_WRITES_PRICE;

  // Firewall (Rate Limiting)
  const rateLimitingCost =
    Math.max(
      0,
      inputs.rateLimitingRequests * 1_000_000 - RATE_LIMITING_INCLUDED
    ) * RATE_LIMITING_PRICE;
  ...
  return {
    ...
    breakdown: [
      ...
      {
        label: "Edge Requests",
        value: edgeRequestsCost,
        category: "Edge Network",
        included: EDGE_REQUESTS_INCLUDED,
      },
      {
        label: "Fast Data Transfer",
        value: fastDataTransferCost,
        category: "Edge Network",
        included: `${FAST_DATA_TRANSFER_INCLUDED} GB`,
      },
      {
        label: "Fast Origin Transfer",
        value: fastOriginTransferCost,
        category: "Edge Network",
        included: `${FAST_ORIGIN_TRANSFER_INCLUDED} GB`,
      },
      {
        label: "Functions Active CPU",
        value: functionActiveCpuCost,
        category: "Compute",
        included: `${FUNCTION_ACTIVE_CPU_INCLUDED} hrs`,
      },
      {
        label: "Functions Memory",
        value: functionProvisionedMemoryCost,
        category: "Compute",
        included: `${FUNCTION_PROVISIONED_MEMORY_INCLUDED} GB-hrs`,
      },
      {
        label: "Function Invocations",
        value: functionInvocationsCost,
        category: "Compute",
        included: FUNCTION_INVOCATIONS_INCLUDED,
      },
      {
        label: "Sandbox Active CPU",
        value: sandboxActiveCpuCost,
        category: "Sandbox",
        included: `${SANDBOX_ACTIVE_CPU_INCLUDED} hrs`,
      },
      {
        label: "Sandbox Memory",
        value: sandboxProvisionedMemoryCost,
        category: "Sandbox",
        included: `${SANDBOX_PROVISIONED_MEMORY_INCLUDED} GB-hrs`,
      },
      {
        label: "Sandbox Creations",
        value: sandboxCreationsCost,
        category: "Sandbox",
        included: SANDBOX_CREATIONS_INCLUDED,
      },
      {
        label: "Sandbox Network",
        value: sandboxNetworkCost,
        category: "Sandbox",
        included: `${SANDBOX_NETWORK_INCLUDED} GB`,
      },
      {
        label: "Workflow Steps",
        value: workflowStepsCost,
        category: "Workflow",
        included: WORKFLOW_STEPS_INCLUDED,
      },
      {
        label: "Workflow Storage",
        value: workflowStorageCost,
        category: "Workflow",
        included: `${WORKFLOW_STORAGE_INCLUDED} GB`,
      },
      {
        label: "Build Standard Machines",
        value: standardMachineCost,
        category: "Build",
      },
      {
        label: "Build Enhanced Machines",
        value: enhancedMachineCost,
        category: "Build",
      },
      {
        label: "Build Turbo Machines",
        value: turboMachineCost,
        category: "Build",
      },
      {
        label: "Image Transformations",
        value: imageTransformationsCost,
        category: "Image Optimization",
        included: IMAGE_TRANSFORMATIONS_INCLUDED,
      },
      {
        label: "Image Cache Reads",
        value: imageCacheReadsCost,
        category: "Image Optimization",
        included: IMAGE_CACHE_READS_INCLUDED,
      },
      {
        label: "Image Cache Writes",
        value: imageCacheWritesCost,
        category: "Image Optimization",
        included: IMAGE_CACHE_WRITES_INCLUDED,
      },
      { label: "Web Analytics", value: analyticsCost, category: "Analytics" },
      {
        label: "Blob Storage Size",
        value: blobStorageCost,
        category: "Blob Storage",
        included: `${BLOB_STORAGE_INCLUDED} GB`,
      },
      {
        label: "Blob Simple Ops",
        value: blobSimpleOpsCost,
        category: "Blob Storage",
        included: BLOB_SIMPLE_OPS_INCLUDED,
      },
      {
        label: "Blob Advanced Ops",
        value: blobAdvancedOpsCost,
        category: "Blob Storage",
        included: BLOB_ADVANCED_OPS_INCLUDED,
      },
      {
        label: "Blob Data Transfer",
        value: blobDataTransferCost,
        category: "Blob Storage",
        included: `${BLOB_DATA_TRANSFER_INCLUDED} GB`,
      },
      {
        label: "Edge Config Reads",
        value: edgeConfigReadsCost,
        category: "Edge Config",
        included: EDGE_CONFIG_READS_INCLUDED,
      },
      {
        label: "Edge Config Writes",
        value: edgeConfigWritesCost,
        category: "Edge Config",
        included: EDGE_CONFIG_WRITES_INCLUDED,
      },
      {
        label: "ISR Reads",
        value: isrReadsCost,
        category: "ISR",
        included: ISR_READS_INCLUDED,
      },
      {
        label: "ISR Writes",
        value: isrWritesCost,
        category: "ISR",
        included: ISR_WRITES_INCLUDED,
      },
      {
        label: "Rate Limiting",
        value: rateLimitingCost,
        category: "Firewall",
        included: RATE_LIMITING_INCLUDED,
      },
...
export const vercelConfig: ProviderConfig<VercelInputs, VercelPlan> = {
  ...
  calculate: (inputs, plan, teamMembers = 1) =>
    calculateVercelCost(inputs, plan, teamMembers),
