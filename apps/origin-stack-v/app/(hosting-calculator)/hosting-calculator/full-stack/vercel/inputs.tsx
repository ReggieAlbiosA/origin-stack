...
export function VercelInputs() {
  const { inputs, setInputs, plan, teamMembers, setTeamMembers } =
    useCalculator<VercelInputs, VercelPlan>();
  ...
  const edgeNetworkInputs = [
    {
      ...
      description:
        "Number of HTTP requests served from Vercel's global CDN/edge network to clients.",
      usedBy:
        "Static pages (SSG), cached API routes, ISR pages, SSR requests, Edge Functions, and any request served through Vercel's CDN. Fresh API requests count if configured with Edge Runtime instead of Node.js runtime.",
    },
    {
      ...
      description:
        "Total bandwidth transferred from Vercel's CDN to end users (egress traffic).",
      usedBy:
        "HTML pages, JavaScript bundles, CSS files, images, API responses, and any asset served from your deployment. Increases with page size × visitor count.",
    },
    {
      ...
      description:
        "Bandwidth transferred from your origin server/external APIs to Vercel's infrastructure.",
      usedBy:
        "ISR revalidations fetching from CMS/database, Server-Side Rendering calling external APIs, Serverless Functions fetching from third-party services.",
    },
  ...
  const functionsInputs = [
    {
      ...
      description:
        "Total CPU execution time consumed by your serverless functions.",
      usedBy:
        "API Routes, Server-Side Rendering (SSR), Server Actions, Middleware, Background Jobs, and any compute-intensive operations in Serverless/Edge Functions.",
    },
    {
      ...
      description:
        "Amount of RAM allocated to functions × execution duration (GB-hours).",
      usedBy:
        "All Serverless Functions. Higher memory needed for: image processing, PDF generation, large data transformations, ML inference, or concurrent operations.",
    },
    {
      key: "invocations",
      ...
      description:
        "Number of times your serverless functions are invoked/executed.",
      usedBy:
        "Every API request, Server Action call, Middleware execution, or background job trigger. Each function execution counts as one invocation.",
    },
  ] satisfies InputConfig[];
  ...
  const buildAnalyticsInputs = [
    ...
    },
    {
      ...
      description:
        "Number of user interactions tracked by Vercel Analytics (privacy-friendly, no cookies).",
      usedBy:
        "Page views, clicks, custom events. Provides Core Web Vitals, audience insights, and real-user monitoring without GDPR concerns.",
    },
  ] satisfies InputConfig[];
  ...
  const imageOptimizationInputs = [
    {
      ...
      description:
        "Number of times cached/optimized images are served to users.",
      usedBy:
        "Next.js Image component serving already-optimized images from cache. Occurs on repeat visits, cached pages, or when images are already transformed.",
    },
    {
      ...
      description:
        "Number of new image transformations/optimizations performed and stored in cache.",
      usedBy:
        "First-time image requests requiring: format conversion (WebP/AVIF), resizing, quality adjustment, or responsive variant generation through next/image.",
    },
    {
      ...
      description:
        "Number of unique image transformations/variants created per month.",
      usedBy:
        "Resizing, format conversion (WebP/AVIF), quality adjustments, cropping operations. Each unique combination (size + format + quality) counts once.",
    },
  ] satisfies InputConfig[];
  ...
  const blobStorageInputs = [
    {
      ...
      description:
        "Total storage space occupied by files in Vercel Blob (S3-compatible object storage).",
      usedBy:
        "User-uploaded images/documents, generated PDFs/reports, video files, backups, or any binary data stored via @vercel/blob SDK.",
    },
    {
      key: "blobSimpleOps",
      ...
      usedBy:
        "GET requests to serve files, downloading attachments, streaming video, or accessing stored assets via blob URLs.",
    },
    {
      key: "blobAdvancedOps",
      ...
      usedBy:
        "PUT/POST requests for file uploads, DELETE for cleanup, multipart uploads for large files, or metadata updates.",
    },
    {
      ...
      description:
        "Bandwidth used transferring data to/from Vercel Blob storage.",
      usedBy:
        "Uploading files from clients, downloading large assets, video streaming, or serving user-generated content from blob storage.",
    },
  ] satisfies InputConfig[];
  ...
  const edgeConfigInputs = [
    {
      ...
      description:
        "Number of times configuration data is read from Edge Config (ultra-low-latency key-value store).",
      usedBy:
        "Feature flag checks, A/B test variants, geo-based routing rules, maintenance mode toggles, or dynamic config loaded in Middleware/Edge Functions.",
    },
    {
      ...
      usedBy:
        "Updating feature flags, changing A/B test percentages, enabling/disabling features, or modifying routing rules via dashboard or API without redeployment.",
    },
  ] satisfies InputConfig[];
  ...
  const isrInputs = [
    {
      ...
      description:
        "Number of times pre-generated ISR pages are served from cache.",
      usedBy:
        "Next.js pages with revalidate or on-demand revalidation. Serves cached static HTML until revalidation period expires or manual revalidation triggered.",
    },
    {
      key: "isrWrites",
      label: "ISR Writes",
      ...
      usedBy:
        "Background revalidation after revalidate time expires, on-demand revalidation via revalidatePath/revalidateTag, or CMS webhook-triggered updates.",
    },
  ] satisfies InputConfig[];
  ...
  const sandboxInputs = [
    ...
    },
    {
      ...
      description:
        "Amount of RAM allocated to sandbox environments × execution duration.",
      usedBy: undefined,
    },
    {
  ...
  const firewallInputs = [
    {
      ...
      description:
        "Total requests that pass rate limiting checks (requests within allowed limits).",
      usedBy:
        "Protecting API endpoints, login pages, form submissions. Charges apply for requests that are evaluated and allowed (not for blocked requests).",
    },
  ] satisfies InputConfig[];
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                Pro plan: $20/user/month + $20 credit/user
              </p>
            </div>
          </div>
          <InputSlider
      ...
      <InputGroup title="Edge Network & CDN" icon={<Zap />}>
        {edgeNetworkInputs.map((input) => (
          <InputSection
            ...
            onValueChange={(v) =>
              setInputs((prev) => ({ ...prev, [input.key]: v }))
            }
            min={input.min}
            max={input.max}
            step={input.step}
      ...
      <InputGroup title="Vercel Firewall" icon={<Shield />}>
        ...
        <InputSection
          ...
          onValueChange={(v) =>
            setInputs((prev) => ({ ...prev, botId: v > 0 ? v : undefined }))
          }
          min={0}
          max={100_000}
          step={1000}
